...
public:
    ...
    void apply(int n){ //n is number of iterations
        float delta_x = 0;
        float delta_y = 0;
        float sum_x = 0;
        float sum_y = 0;
        ...
        for(int a = 0; a < elastic_net.index_elastic_points; a++){
            ...
            delta_x += beta * calculate_k(n) * sum_x;
            delta_y += beta * calculate_k(n) * sum_y;
    ...
    float calculate_v_i_a(int i, int a, int n){
        float sum = 0;
        for(int b = 0; b < elastic_net.index_elastic_points; b++){
            sum += exp(-(pow(elastic_net.cities[i].x_coord - elastic_net.elastic_net_points[b].x_coord, 2)+
                         pow(elastic_net.cities[i].x_coord - elastic_net.elastic_net_points[b].x_coord, 2))/2*pow(calculate_k(n),2));
        }

        float result = (exp(-(pow(elastic_net.cities[i].x_coord - elastic_net.elastic_net_points[a].x_coord, 2)+
                             pow(elastic_net.cities[i].x_coord - elastic_net.elastic_net_points[a].x_coord, 2))/2*pow(calculate_k(n),2)))/sum;

        return result;
    }

    float calculate_k(int n){
        float k_n = fmax(pow(0.99, floor(n/50))*k, 0.01);
